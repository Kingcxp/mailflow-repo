"""Feishu notifier: pushes mail summaries to Feishu.
...
logger = logging.getLogger("mailflow.notification.feishu")

PLUGIN_INFO = PluginInfo(
    plugin_id="mailflow-notify-feishu",
    name="Feishu Notifier",
    version="0.1.0",
    description="Push mail alerts to a Feishu custom-bot webhook",
    kinds=[ComponentKind.NOTIFIER],
)


class FeishuNotifier:
    """Feishu delivery channel. Options documented in plugin.json."""
    ...
    def _payload(self, record: MailRecord) -> dict[str, Any]:
        return {
            "msg_type": "text",
            "content": {"text": self._summary_text(record)},
        }
    ...
    async def notify(self, record: MailRecord) -> None:
        ...
        if not url:
            logger.warning("%(log)s notifier has no endpoint; skipping", {"log": "feishu"})
            return
...
class FeishuNotifierPlugin:
    def mailflow_plugin_info(self) -> PluginInfo:
        return PLUGIN_INFO

    def mailflow_register(self, registrar: PluginRegistrar, config: Any) -> None:
        registrar.add_notifier("feishu", FeishuNotifier)


plugin = FeishuNotifierPlugin()

__all__ = ["FeishuNotifier", "FeishuNotifierPlugin", "plugin"]
