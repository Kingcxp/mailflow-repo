"""Discord notifier: pushes mail summaries to Discord.
...
logger = logging.getLogger("mailflow.notification.discord")

PLUGIN_INFO = PluginInfo(
    plugin_id="mailflow-notify-discord",
    name="Discord Notifier",
    version="0.1.0",
    description="Push mail alerts to a Discord webhook",
    kinds=[ComponentKind.NOTIFIER],
)


class DiscordNotifier:
    """Discord delivery channel. Options documented in plugin.json."""
    ...
    def _payload(self, record: MailRecord) -> dict[str, Any]:
        payload = {"content": self._summary_text(record)}
        username = str(self._options.get("username", ""))
        if username:
    ...
    async def notify(self, record: MailRecord) -> None:
        ...
        if not url:
            logger.warning("%(log)s notifier has no endpoint; skipping", {"log": "discord"})
            return
...
class DiscordNotifierPlugin:
    ...
    def mailflow_register(self, registrar: PluginRegistrar, config: Any) -> None:
        registrar.add_notifier("discord", DiscordNotifier)


plugin = DiscordNotifierPlugin()

__all__ = ["DiscordNotifier", "DiscordNotifierPlugin", "plugin"]
