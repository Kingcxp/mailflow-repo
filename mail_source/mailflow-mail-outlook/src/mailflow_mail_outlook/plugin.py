"""Outlook Mail Source mail source: IMAP poll + SMTP reply.
...
Outlook Mail Source requires an app password, not the account password — the
...
logger = logging.getLogger("mailflow.mail.outlook")
...
class OutlookSource:
    """Polls the Outlook Mail Source IMAP INBOX and sends replies over SMTP."""
        ...
        try:
            ...
            for uid_int in wanted:
                ...
                if not fetch or fetch[0] is None:
                    break
                mail = parse_mime(bytes(fetch[0][1]), self._account.account_id, provider="outlook")
        ...
        try:
            ...
            for uid_int in window:
                ...
                if not fetch or fetch[0] is None:
                    continue
                messages.append(
                    parse_mime(bytes(fetch[0][1]), self._account.account_id, provider="outlook")
                )
            return messages
        finally:
            with contextlib.suppress(Exception):
...
PLUGIN_INFO = PluginInfo(
    plugin_id="mailflow-mail-outlook",
    name="Outlook Mail Source",
    version="0.1.0",
    description="IMAP/SMTP mail source for Outlook/Office 365 with app-password auth",
...
class OutlookPlugin:
    ...
    def mailflow_register(self, registrar: PluginRegistrar, config: Any) -> None:
        registrar.add_source("outlook", OutlookSource)


plugin = OutlookPlugin()

__all__ = ["OutlookPlugin", "OutlookSource", "parse_mime", "plugin"]
