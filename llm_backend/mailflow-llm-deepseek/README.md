...
MailFlow at DeepSeek. Registers the LLM backend component id `deepseek`.
...
Both are used only when the `[[llms]]` entry does not set them; an explicit
`model` or `base_url` always wins.

### Authentication

Set `api_key` directly or reference an environment variable with
`api_key_env` (e.g. `DEEPSEEK_API_KEY`). MailFlow expands `${VAR}`
...
DeepSeek。注册的大模型后端组件 id 为 `deepseek`。
...
默认值：`base_url` 为 `https://api.deepseek.com/v1`，`model` 为
`deepseek-chat`。仅当 `[[llms]]` 条目未设置时才使用默认值；显式配置的
`model` 或 `base_url` 始终优先。

### 鉴权

直接设置 `api_key`，或用 `api_key_env` 引用环境变量（例如
`DEEPSEEK_API_KEY`）。MailFlow 在加载配置时会展开 `${VAR}` 占位符。
...
Metadata: [`plugin.json`](plugin.json) · Marketplace: [https://github.com/Kingcxp/mailflow-repo/tree/main/llm_backend/mailflow-llm-deepseek](https://github.com/Kingcxp/mailflow-repo/tree/main/llm_backend/mailflow-llm-deepseek)
