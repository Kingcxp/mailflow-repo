"""DeepSeek LLM backend (OpenAI-compatible chat completions).

Component id ``deepseek``. Thin config preset over the OpenAI-compatible
Chat Completions transport:

    POST https://api.deepseek.com/v1/chat/completions
    Authorization: Bearer <api key>

Defaults: base_url ``https://api.deepseek.com/v1``, model ``deepseek-chat``.
The standard library's urllib performs the POST on a worker thread, so the
plugin has no third-party runtime dependency beyond ``mailflow-core``.
Only transient failures — timeouts, transport errors, HTTP 408/429/5xx — are
retried, with exponential backoff capped at 5 s. Error text is sanitized so
no URL or key material ever surfaces.
"""
...
logger = logging.getLogger("mailflow.llm.deepseek")
...
PLUGIN_INFO = PluginInfo(
    plugin_id="mailflow-llm-deepseek",
    name="DeepSeek LLM Backend",
    version="0.1.0",
    description="OpenAI-compatible chat completions transport for DeepSeek (component id: deepseek)",
...
class LLMPlugin:
    ...
    def mailflow_register(self, registrar: PluginRegistrar, config) -> None:
        registrar.add_llm("deepseek", DeepSeekBackend)


plugin = LLMPlugin()

__all__ = ["DeepSeekBackend", "LLMPlugin", "OpenAICompatBackend", "plugin"]
