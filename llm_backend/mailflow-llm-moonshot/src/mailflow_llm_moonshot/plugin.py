"""Moonshot LLM backend (OpenAI-compatible chat completions).

Component id ``moonshot``. Thin config preset over the OpenAI-compatible
...
logger = logging.getLogger("mailflow.llm.moonshot")
...
class MoonshotBackend(OpenAICompatBackend):
    backend_id = "moonshot"
...
PLUGIN_INFO = PluginInfo(
    plugin_id="mailflow-llm-moonshot",
    name="Moonshot LLM Backend",
    version="0.1.0",
    description="OpenAI-compatible chat completions transport for Moonshot AI (component id: moonshot)",
...
class LLMPlugin:
    ...
    def mailflow_register(self, registrar: PluginRegistrar, config) -> None:
        registrar.add_llm("moonshot", MoonshotBackend)


plugin = LLMPlugin()

__all__ = ["LLMPlugin", "MoonshotBackend", "OpenAICompatBackend", "plugin"]
