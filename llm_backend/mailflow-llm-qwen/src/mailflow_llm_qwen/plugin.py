"""Qwen LLM backend (OpenAI-compatible chat completions).

Component id ``qwen``. Thin config preset over the OpenAI-compatible
...
model ``qwen-plus``. The standard library's urllib performs the POST on a
...
logger = logging.getLogger("mailflow.llm.qwen")
...
PLUGIN_INFO = PluginInfo(
    plugin_id="mailflow-llm-qwen",
    name="Qwen LLM Backend",
    version="0.1.0",
    description="OpenAI-compatible chat completions transport for Alibaba Qwen (component id: qwen)",
...
class LLMPlugin:
    ...
    def mailflow_register(self, registrar: PluginRegistrar, config) -> None:
        registrar.add_llm("qwen", QwenBackend)


plugin = LLMPlugin()

__all__ = ["LLMPlugin", "OpenAICompatBackend", "QwenBackend", "plugin"]
