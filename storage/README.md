...
| Plugin | Description |
|---|---|
| _No plugins yet — yours could be the first._ | |

### Adding a plugin

Create a folder `storage/<plugin-id>/` with `plugin.json` and the plugin source, then open a pull request. The PR workflow validates it automatically.
